...
const EmailToken = require("../models/EmailToken"); // asume modelo existente
const { transporter } = require("../config/mailer"); // asume config existente
const { getJwtSecret } = require("../config/auth");

/* ========= Config ========= */
const isProd = process.env.NODE_ENV === "production";
const COOKIE_NAME = (process.env.SESSION_COOKIE_NAME || "sid").trim();
const COOKIE_DOMAIN =
...
const CLIENT_URL = (process.env.CLIENT_URL || "https://ssselvasagrada.com").replace(/\/+$/, "");
...
function cookieOptions() {
  ...
  if (isProd) {
    base.secure = true;
    base.sameSite = "lax"; // api.<dominio> y <dominio> son same-site
  ...
  async (req, res, next) => {
    try {
      ...
      try {
        ...
        // Log suave, no bloquear registro por fallo de mail
...
exports.verifyEmail = async (req, res, next) => {
  try {
    ...
    if (!token) {
      if (req.method === "GET") {
        return res.redirect(`${CLIENT_URL}/verify-email?status=missing`);
      }
    ...
    if (!rec || rec.expiresAt < new Date()) {
      if (req.method === "GET") {
        return res.redirect(`${CLIENT_URL}/verify-email?status=invalid`);
      }
...
exports.login = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign(
      { sub: u._id.toString(), id: u._id.toString(), role: u.role || "user" },
      JWT_SECRET,
      { expiresIn: "7d" }
    );
