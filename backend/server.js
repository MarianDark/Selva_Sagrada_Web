...
// Express 5: catch-all con RegExp, no strings con wildcards
...
const logger = pino(
  {
    ...
    // Nunca imprimas credenciales en logs
    redact: {
      paths: [
        "req.headers.cookie",
...
// pino-http debe ir ANTES de las rutas para capturarlo todo
app.use(
  pinoHttp({
    logger,
    // No queremos IDs aleatorios distintos si ya viene un x-request-id
    genReqId: (req) =>
      req.headers["x-request-id"] || Math.random().toString(36).slice(2),
    // Calla rutas ruidosas y las preflight
    autoLogging: {
      ignore: (req) =>
        req.method === "OPTIONS" ||
        IGNORED_ROUTES.some((p) => req.url.startsWith(p)),
    },
    // En prod solo loguea lo serio: 4xx warn, 5xx error. 2xx/3xx = silencio.
    customLogLevel: (res, err) => {
      if (err || res.statusCode >= 500) return "error";
      if (res.statusCode >= 400) return "warn";
      return isProd ? "silent" : "info";
    },
    // Reduce ruido en logs (solo lo útil)
    serializers: {
      req(req) {
        return {
          id: req.id,
          ...
          // headers sensibles ya se redactan arriba
        };
      },
      res(res) {
        return {
