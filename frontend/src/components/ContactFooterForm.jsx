...
export default function ContactFooterForm() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Ejecuta el captcha invisible y obtiene el token
      const captchaToken = await captchaRef.current.executeAsync();
      // Resetea el widget para la próxima vez
      ...
      // Mensaje de error más claro si viene del backend
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
      ...
      {/* reCAPTCHA invisible. No hace falta script global si usas este componente */}
      <ReCAPTCHA
        ref={captchaRef}
