...
function InstagramIcon(props){ /* ...igual que tenías... */ }
function FacebookIcon(props){ /* ...igual que tenías... */ }
function WhatsIcon(props){ /* ...igual que tenías... */ }

export default function Footer() {
  const { canInstall, promptInstall } = useA2HS()

  return (
    <footer className="mt-12 bg-jungle-900 text-zinc-200 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-5 text-xs text-zinc-300 flex flex-col items-center gap-3">
        ...
        {canInstall && (
          <button onClick={promptInstall} className="px-4 py-2 rounded-lg border border-emerald-400 text-emerald-200 font-semibold hover:bg-white/10">Instalar app</button>
        )}
